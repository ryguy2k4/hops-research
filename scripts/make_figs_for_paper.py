...
def make_single_plot(target_name, output_name, rows=1, cols=2, figsize=None):
    if figsize == None:
        figure = plt.figure(figsize=(17/3*cols, 22/5*rows))
    else:
        figure = plt.figure(figsize=figsize)
    ...
    # verify output path exists and
    # skip already existing files if you don't want to overwrite them
    field = by_field[by_field['field'] == target_name]
    ...
    ### MOMENT MAP PANEL
    # M8 Map
    print(field['integrated_channels'])
    print(type(field))
    print(type(field['integrated_channels']))
    if field['integrated_channels'].isna().any():
        print("M8")   
        # Create Figure
        fig = create_m8_map(hdu_spw39, distance=distance, figure=figure, subplot=(rows, cols, 1), multiimage=True, use_offset_labels=True)
        fig.set_title(f"{target_name} M8")
    ...
    # M0 Map
        ...
        outflow = outflow_data.loc[outflow_data['field'] == target_name].groupby('field').first().reset_index()
        channels = getIdx([outflow.at[0, 'red_channels'], outflow.at[0, 'blue_channels']])
        fig = create_m0_map(hdu_spw39, channels, sigma=3, distance=distance, figure=figure, subplot=(rows, cols, 1), multiimage=True, use_offset_labels=True)
        fig.set_title(f"{target_name} M0")
    ...
    size = np.array([3, 3]) * u.arcsecond
    center = SkyCoord(target_info[target_info['important']==True]['RA'].mean(), target_info[target_info['important']==True]['Dec'].mean(), unit=u.degree)
    fig1 = create_cont_map(hdu_cont, size=size, center=center, distance=distance, scalebar_au=100, figure=figure, subplot=(rows, cols, 2), multiimage=True, use_offset_labels=True)
...
# SET OUTPUT
output_folder = "results/maps_for_paper"
if not os.path.exists(output_folder):
    os.mkdir(output_folder)

if not os.path.exists(os.path.join(output_folder, 'figset')):
    os.mkdir(os.path.join(output_folder, 'figset'))

if not os.path.exists(os.path.join(output_folder, 'arxiv')):
    os.mkdir(os.path.join(output_folder, 'arxiv'))
...
app_full = by_field.sort_values('ra')['field'].values.tolist()

# GENERATE FIGURES
# Figs 1 & 2 for Main Body of Paper
...
# Figure Set & Example Figure for Online Journal
make_compound_plots(app_full[0:4], "figset/fig-5", rows=4, cols=2)
for i in range(1, len(app_full)+1):
    make_single_plot(app_full[i-1], f"figset/f5_{i}")

# 13 Appendix Figures for arXiv Manuscript
make_compound_plots(app_full[0:4], "arxiv/appendix-1", rows=4, cols=2)
make_compound_plots(app_full[4:8], "arxiv/appendix-2", rows=4, cols=2)
make_compound_plots(app_full[8:12], "arxiv/appendix-3", rows=4, cols=2)
make_compound_plots(app_full[12:16], "arxiv/appendix-4", rows=4, cols=2)
make_compound_plots(app_full[16:20], "arxiv/appendix-5", rows=4, cols=2)
make_compound_plots(app_full[20:24], "arxiv/appendix-6", rows=4, cols=2)
make_compound_plots(app_full[24:28], "arxiv/appendix-7", rows=4, cols=2)
make_compound_plots(app_full[28:32], "arxiv/appendix-8", rows=4, cols=2)
make_compound_plots(app_full[32:36], "arxiv/appendix-9", rows=4, cols=2)
make_compound_plots(app_full[36:40], "arxiv/appendix-10", rows=4, cols=2)
make_compound_plots(app_full[40:44], "arxiv/appendix-11", rows=4, cols=2)
make_compound_plots(app_full[44:48], "arxiv/appendix-12", rows=4, cols=2)
make_compound_plots(app_full[48::], "arxiv/appendix-13", rows=4, cols=2)